...
class ttnMQTT(object):
  ...
  def __init__(self, weather_station, commit = False,**ttn_params):
    ...
    self.client_userdata = {'weather_station' : weather_station, 'commit' : commit, 'ttnMQTT': self }
  ...
  def process_link(self, direction='UPLINK', data = ''):
    ...
    def on_message(client, userdata,message):
      ...
      weather_station = userdata['weather_station'] # weatherStation object is in userdata
    ...
    def on_disconnect(client, userdata, rc):
      ...
      if rc != 0:
          raise ConnectionError("Unexpected disconnection from mqtt. Result: " + str(rc))     
    ...
    mqttc.username_pw_set(self.ttn_params['user'], self.ttn_params['password'])

    mqttc.tls_set()

    # connect

    mqttc.connect(self.ttn_params['public_tls_address'], int(self.ttn_params['public_tls_address_port']), 60)
    ...
    if direction == 'UPLINK':
      ...
      try:
       ...
       topic = "v3/" + self.ttn_params['user'] + "/devices/" + self.ttn_params['device_id'] + "/down/push"
       #breakpoint()
       self.logger.info("Subscribing to topic %s with QOS: %d", topic, qos)
       ...
       if data != '':
         ...
         if status == 0:
             ...
             self.logger.info("Failed to send message to topic %s", topic)
...
class weatherStation(object):
  ...
  def _get_stations(self,**postgres_params):
    ...
    try:
        ...
        pcur.execute("SELECT id,name, latitude, longitude, altitude, eu_id  FROM weather_station")
  ...
  def parse_data(self, parsed_json):
    ...
    if not message_type in self.VALID_MESSAGES:
      ...
      self.logger.warning("Message is not recognised - stopping parse:  %s", payload)
      return d
    ...
    d ['timestamp'] = (ts.strftime('%Y-%m-%d %H:%M:%S')) + "+00:00" # time from stations is always UTC
    ...
    if message_type == 100: # weather report 
      ...
      d ['wind_speed'] = int(payload[WIND_SPEED],16)/100 # all but directions have 2 implied decimals 
